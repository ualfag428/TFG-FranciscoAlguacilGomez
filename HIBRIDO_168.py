...
def create_windows_multi(dataset, window_size=168, horizon=1, salto=1):  
    X, y = [], []
...
print(f"\nINICIANDO PREDICCIÓN AUTOREGRESIVA ({N_PASOS} pasos, {N_MC} trayectorias MC)")

ventana_mc = np.tile(X_test[0][np.newaxis, :, :], (N_MC, 1, 1))
...
real_temp, real_presion, real_humedad, real_viento, real_rad = inverse_multi(
    scaler, y_test[:N_PASOS], n_features)
...
plt.tight_layout()
plt.savefig("Prueba_Simple_MC_168_v2C.png", dpi=300, bbox_inches='tight')
plt.show()
...
for j, nombre in enumerate(nombres):
    ...
    # ECE en espacio escalado
    real_sc = y_test[:N_PASOS, j]
...
print(f"\n{'='*65}")
print("TABLA 3 — DETECCIÓN DE EVENTOS EXTREMOS (Hit Rate con umbral)")
print(f"{'='*65}")
...
# VISUALIZACIÓN PRIMERAS 48 HORAS


H48 = 48

upper_48_scaled = pred_mean_global[:H48] + 1.96 * pred_std_global[:H48]
lower_48_scaled = pred_mean_global[:H48] - 1.96 * pred_std_global[:H48]

pred_t48, pred_p48, pred_h48, pred_v48, _ = inverse_multi(scaler, pred_mean_global[:H48], n_features)
real_t48, real_p48, real_h48, real_v48, _ = inverse_multi(scaler, y_test[:H48],            n_features)
upper_t48, upper_p48, upper_h48, upper_v48, _ = inverse_multi(scaler, upper_48_scaled, n_features)
lower_t48, lower_p48, lower_h48, lower_v48, _ = inverse_multi(scaler, lower_48_scaled, n_features)

x48 = np.arange(H48)

fig, axs = plt.subplots(2, 2, figsize=(20, 10), sharex=True)
fig.suptitle('Predicción primeras 48 horas — Modelo Híbrido 168h', fontsize=14, fontweight='500')

def plot_48(ax, real, pred, upper, lower, color, title, ylabel):
    ax.plot(x48, real,  color='black', alpha=0.5, label='Real', linewidth=1)
    ax.plot(x48, pred,  color=color, linestyle='--', label='Predicción media', linewidth=1.5)
    ax.fill_between(x48, lower, upper, color=color, alpha=0.2, label='IC 95%')
    ax.set_title(title)
    ax.set_ylabel(ylabel)
    ax.set_xlabel('Horas predichas')
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)
    # Línea vertical a las 24h
    ax.axvline(x=24, color='gray', linestyle=':', alpha=0.6, linewidth=1)
    ax.text(24.5, ax.get_ylim()[0], '24h', fontsize=8, color='gray', va='bottom')

plot_48(axs[0, 0], real_t48, pred_t48, upper_t48, lower_t48, 'red',    'Temperatura (°C)',       '°C')
plot_48(axs[0, 1], real_p48, pred_p48, upper_p48, lower_p48, 'orange', 'Presión (hPa)',          'hPa')
plot_48(axs[1, 0], real_h48, pred_h48, upper_h48, lower_h48, 'green',  'Humedad (%)',            '%')
plot_48(axs[1, 1], real_v48, pred_v48, upper_v48, lower_v48, 'blue',   'Velocidad viento (m/s)', 'm/s')

plt.tight_layout()
plt.savefig("Primeras_48h_168h.png", dpi=300, bbox_inches='tight')
plt.show()


# MÉTRICAS — PRIMERAS 48 HORAS

print(f"\n{'='*85}")
print("MÉTRICAS PRIMERAS 48 HORAS — Modelo Híbrido 168h")
print(f"{'='*85}")

std_t48 = upper_t48 - pred_t48
std_p48 = upper_p48 - pred_p48
std_h48 = upper_h48 - pred_h48
std_v48 = upper_v48 - pred_v48

pred_vars_48 = [pred_t48, pred_p48, pred_h48, pred_v48]
real_vars_48 = [real_t48, real_p48, real_h48, real_v48]
upper_vars_48 = [upper_t48, upper_p48, upper_h48, upper_v48]
lower_vars_48 = [lower_t48, lower_p48, lower_h48, lower_v48]
std_vars_48   = [std_t48,   std_p48,   std_h48,   std_v48]
nombres_48    = ["TEMPERATURA", "PRESION", "HUMEDAD", "VIENTO"]

#Tabla 1 - Predicción puntual 48h
print(f"\n{'─'*85}")
print("TABLA 1 — PREDICCIÓN PUNTUAL (primeras 48h)")
print(f"{'─'*85}")
print(f"{'VARIABLE':<12} | {'MAE':<7} | {'RMSE':<7} | {'R²':<7} | {'Bias':<8} | {'MAPE %':<8} | {'Skill':<7} | {'MAE p90↑':<9} | {'MAE p10↓'}")
print("-" * 85)

for j, nombre in enumerate(nombres_48):
    real = real_vars_48[j]
    pred = pred_vars_48[j]
...
# Tabla 2 — Banda de incertidumbre 48h 
print(f"\n{'─'*65}")
print("TABLA 2 — BANDA DE INCERTIDUMBRE (primeras 48h)")
print(f"{'─'*65}")
print(f"{'VARIABLE':<12} | {'PICP %':<8} | {'PINAW':<8} | {'MPIW':<8} | {'Winkler':<10} | {'ECE %'}")
print("-" * 65)
...
for j, nombre in enumerate(nombres_48):
    ...
    pred  = pred_vars_48[j]
    std   = std_vars_48[j]
    upper = upper_vars_48[j]
    lower = lower_vars_48[j]

    real_sc = y_test[:H48, j]
    mu_sc   = pred_mean_global[:H48, j]
    std_sc  = pred_std_global[:H48, j]
