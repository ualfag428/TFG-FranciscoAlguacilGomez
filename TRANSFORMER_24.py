...
# 2. Ventanas y split (sin leakage)
...
# 4. MODELO — TRANSFORMER PURO


input_layer = layers.Input(shape=(window_size, n_features))

#  ampliado para compensar la ausencia del LSTM
...
params = layers.Dense(tfp.layers.IndependentNormal.params_size(5))(x)
...
# FUNCIÓN DE PÉRDIDA — idéntica al híbrido
...
def plot_v(ax, real, pred, upper, lower, color, title, ylabel):
    ...
    ax.fill_between(x, l, u, color=color, alpha=0.2, label='IC 95% (MC propagado)')
    ax.set_title(title)
...
#plot_v(axs[4], real_rad,     pred_rad,     upper_rad,     lower_rad,     'purple', 'Radiación (W/m²)',       'W/m²')

plt.xlabel('Horas predichas')
plt.tight_layout()
plt.savefig("Transformer_puro_24h.png", dpi=300, bbox_inches='tight')
plt.show()
...
nombres = ["TEMPERATURA", "PRESION", "HUMEDAD", "VIENTO", "RADIACION"]


# TABLA 1 - Prediccion puntual

print(f"\n{'='*85}")
print("TABLA 1 — PREDICCION PUNTUAL")
print(f"{'='*85}")
...
print("-" * 85)
print("  Bias>0 → sobreestimación | Bias<0 → subestimación")
print("  Skill>0 → supera persistencia | MAE p90↑ = error en el 10% de valores más altos")
...
print("-" * 65)
print("  PICP~95% ideal | PINAW bajo = banda eficiente | ECE~0% = calibración perfecta")
print("  Winkler: penaliza banda ancha + casos fuera del IC (menor = mejor)")
...
print("-" * 65)
print("  Hit Rate: % de eventos reales correctamente detectados")
print("  Falsa Alarma: % de alertas emitidas sin evento real")
...
print("-" * 80)
print("  Calibración perfecta: PICP empírico ≈ nivel nominal en cada columna")
print("  Valores muy superiores al nominal → banda sobreestima incertidumbre")
print("  Valores muy inferiores al nominal → banda infraestima incertidumbre")
